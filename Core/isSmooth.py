...
its middle is the element whose index number is the same when counting from the beginning of the array and from its end;
...
def isSmooth(arr):
    a=len(arr)
    if arr[0]==arr[-1] and a>=2 and a%2==0:
        if arr[a//2-1]+arr[a//2]==arr[0]:
            return True
        return False
    elif arr[0]==arr[-1] and a>=2 and a%2==1:
        if arr[a//2]==arr[0]:
            return True
        return False
    else:
